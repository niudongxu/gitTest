//通过闭包可以访问创建闭包时所处环境中的全部变量。闭包作为函数创建时所处的作用域中的函数和变量。创建‘安全气泡’。通过这种方式，即使创建函数时所处的作用域已经消失，但是函数仍然能够获得执行时所需的全部内容。
...
//JavaScript引擎通过执行上下文栈（调用栈）跟踪函数的执行。每次调用函数时，都会创建新的函数执行上下文，并推入调用栈顶端。当函数执行完成之后，对应的执行上下文将从调用栈中推出。
